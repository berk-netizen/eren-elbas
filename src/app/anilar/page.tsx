...
import { initialMemories } from "@/lib/dummyData";
...
export default function MemoriesPage() {
    const [memories, setMemories, isLoaded] = useLocalStorageState<AppMemory[]>("eren_memories", initialMemories);
    ...
    const handleSave = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (editingMemory) {
            ...
            setMemories([...memories, { id: Date.now().toString(), title, dateISO, description, image: previewImage }]);
        }
        setIsModalOpen(false);
    };

    const activeMemories = memories || [];
    const sortedMemories = [...activeMemories].sort((a, b) => new Date(b.dateISO).getTime() - new Date(a.dateISO).getTime());
    ...
    return (
        <div className="flex flex-col gap-6 relative pb-6">
            {sortedMemories.length === 0 && (
                <div className="text-center text-gray-500 py-10 mt-10">Hiç anı yok. Eklemek için + butonuna tıkla.</div>
            )}
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingMemory ? "Anıyı Düzenle" : "Yeni Anı Ekle"}>
                <form onSubmit={handleSave} className="flex flex-col gap-4 mt-2">
                    ...
                    <Input
                        ...
                        label="Tarih"
                        type="date"
                        defaultValue={editingMemory?.dateISO}
                        required
                    />
                    ...
                    <div className="w-full">
                        ...
                        <textarea
                            ...
                            placeholder="Neler yaptık, nerelere gittik..."
                            required
                        />
                    </div>

                    <div className="w-full">
                        <label className="block text-sm font-medium mb-1.5">Fotoğraf (İsteğe Bağlı, Maks 10MB)</label>
                        <input
                            type="file"
                            accept="image/*"
                            className="hidden"
                        ...
                        {previewImage ? (
                            ...
                            <button
                                ...
                                className="w-full h-32 rounded-xl border-2 border-dashed border-gray-300 dark:border-gray-600 hover:border-primary dark:hover:border-primary flex flex-col items-center justify-center text-gray-500 hover:text-primary transition-colors bg-gray-50 dark:bg-[#2C2C2E]"
                            >
                                <div className="p-3 bg-white dark:bg-black rounded-full shadow-sm mb-2">
                                    <Upload size={24} />
                                </div>
